use chrono::{DateTime, Datelike, Utc, Months};
...
pub fn period_key(timestamp: &DateTime<Utc>, monthly: bool) -> String {
    if monthly { month_key(timestamp) } else { week_key(timestamp) }
}
...
impl GitIgnoreMatcher {
    pub fn new<P: AsRef<Path>>(root: P) -> Self {
        Self { root: root.as_ref().to_path_buf(), cache: HashMap::new() }
    }

    pub fn is_ignored(&mut self, rel_path: &str) -> bool {
        let abs = self.root.join(rel_path);
        let dir_buf: PathBuf = abs.parent().map(|p| p.to_path_buf()).unwrap_or_else(|| self.root.clone());
    ...
    fn get_or_build_for_dir(&mut self, dir: &Path) -> Option<&ignore::gitignore::Gitignore> {
        if !self.cache.contains_key(dir) {
            self.cache.insert(dir.to_path_buf(), self.build_for_dir(dir));
        }
    ...
    fn build_for_dir(&self, dir: &Path) -> Option<ignore::gitignore::Gitignore> {
        ...
        if root_gi.exists() { let _ = builder.add(&root_gi); }
        let info_excl = self.root.join(".git").join("info").join("exclude");
        if info_excl.exists() { let _ = builder.add(&info_excl); }
        ...
        if let Ok(rel) = dir.strip_prefix(&self.root) {
            ...
            for comp in rel.components() {
                ...
                if f.exists() { let _ = builder.add(f); }
            }
        }
        builder.build().ok()
    }
